"""
OI Bias Monitor — Kite WebSocket Bridge  ·  MCX Crude Oil Edition
==================================================================
Supports Nifty, Sensex, and MCX Crude Oil. Configured via environment variables.
...
2.  Set env vars:
        KITE_API_KEY
        KITE_ACCESS_TOKEN
        INDEX           nifty (default) | sensex | crudeoil
3.  Deploy from GitHub — Railway will run:  python crude_oi_bridge.py

Index-specific behaviour
────────────────────────
  INDEX=nifty     → table oi_history_nifty,     exchange NFO,  strike step 50,  hours 09:15–15:30
  INDEX=sensex    → table oi_history_sensex,    exchange BFO,  strike step 100, hours 09:15–15:30
  INDEX=crudeoil  → table oi_history_crudeoil,  exchange MCX,  strike step 50,  hours 09:00–23:30
                    Spot = front-month CRUDEOIL futures (token resolved dynamically at startup)

Crude Oil notes
───────────────
  • MCX Crude Oil options trade on the MCX exchange (instrument type CE/PE, name CRUDEOIL).
  • There is no static spot-index token for crude; the bridge subscribes to the front-month
    futures contract (lowest expiry FUT) and uses its LTP as the underlying price.
  • The spot token is resolved at startup and re-used for the session. If the front month
    rolls during the day, restart the bridge.
  • Market hours 09:00–23:30 IST Mon–Fri (MCX commodity session).
  • Set OI_NUM_STRIKES=11 (default) for ATM ±5 × ₹50 = ±250 around the futures price.
  • Set BEAR_BULL_OFFSET env var (default 100) to adjust bear/bull strike distance.
...
# ── MARKET HOURS ──────────────────────────────────────────────────────────────
# Market hours are set per-index after the INDEX env var is read (see below).
# NSE cash/F&O: Monday–Friday, 09:15–15:30 IST
# MCX Crude Oil: Monday–Friday, 09:00–23:30 IST
# The sentinel values here are overridden immediately after the INDEX block.
MARKET_OPEN_HM  = (9, 15)
MARKET_CLOSE_HM = (15, 30)   # overridden for crudeoil below
...
# ── INDEX SELECTION ───────────────────────────────────────────────────────────
# Set INDEX=nifty (default), INDEX=sensex, or INDEX=crudeoil in Railway env.
_INDEX_RAW = os.environ.get("INDEX", "nifty").strip().lower()
if _INDEX_RAW not in ("nifty", "sensex", "crudeoil"):
    raise ValueError(f"INDEX env var must be 'nifty', 'sensex', or 'crudeoil', got: {_INDEX_RAW!r}")
...
elif _INDEX_RAW == "crudeoil":
    INDEX_NAME      = "crudeoil"
    INDEX_LABEL     = "Crude Oil"
    SPOT_TOKEN      = None        # resolved dynamically at startup (front-month FUT)
    SPOT_QUOTE_KEY  = None        # resolved dynamically at startup
    INSTRUMENT_NAME = "CRUDEOIL"  # matches kite.instruments "name" field for MCX options
    EXCHANGE        = "MCX"
    STRIKE_STEP     = 50          # MCX Crude Oil options strike step = ₹50/bbl
    ROLL_THRESHOLD  = 100
    DB_TABLE        = "oi_history_crudeoil"
...
# ── PER-INDEX MARKET HOURS ────────────────────────────────────────────────────
# MCX Crude Oil trades 09:00–23:30 IST; NSE/BSE trades 09:15–15:30 IST.
if _INDEX_RAW == "crudeoil":
    MARKET_OPEN_HM  = (9,  0)
    MARKET_CLOSE_HM = (23, 30)
elif _INDEX_RAW == "sensex":
    MARKET_OPEN_HM  = (9, 15)
...
# Bear/Bull strike offset from ATM (configurable via env; default 100 for all indices)
BEAR_BULL_OFFSET = int(os.environ.get("BEAR_BULL_OFFSET", 100))
...
def compute_bear_bull(spot):
    ...
    return base - BEAR_BULL_OFFSET, base + BEAR_BULL_OFFSET
...
def get_live_spot():
    ...
    For Nifty/Sensex: fetches the index quote directly.
    For Crude Oil: finds the front-month CRUDEOIL futures contract, subscribes its
                   instrument token as SPOT_TOKEN, then returns its LTP.
    """
    global SPOT_TOKEN, SPOT_QUOTE_KEY

    if INDEX_NAME == "crudeoil":
        # Dynamically resolve front-month CRUDEOIL FUT token (changes monthly)
        ...
        if not futures:
            raise RuntimeError("No CRUDEOIL FUT instruments found on MCX — check Kite instruments dump.")
        front = futures[0]
        SPOT_TOKEN     = front["instrument_token"]
        SPOT_QUOTE_KEY = f"MCX:{front['tradingsymbol']}"
        print(f"  Crude Oil spot: {front['tradingsymbol']}  token={SPOT_TOKEN}  expiry={front['expiry']}")
        quote = kite.quote(SPOT_QUOTE_KEY)
        return quote[SPOT_QUOTE_KEY]["last_price"]

    # Nifty / Sensex — static token and quote key
    quote = kite.quote(SPOT_QUOTE_KEY)
    return quote[SPOT_QUOTE_KEY]["last_price"]
...
if __name__ == "__main__":
    ...
    print(f"Market hours: {MARKET_OPEN_HM[0]:02d}:{MARKET_OPEN_HM[1]:02d} – "
          f"{MARKET_CLOSE_HM[0]:02d}:{MARKET_CLOSE_HM[1]:02d} IST")
    if INDEX_NAME == "crudeoil":
        print("Crude Oil mode: spot token resolved from front-month FUT at startup.")
        print("Set BEAR_BULL_OFFSET env var to adjust bear/bull offset (default 100).")
    print("=" * 60)
    ...
    # At market close the ticker is stopped so no after-hours ticks arrive.
